...
| 判型を指定しなかった | A5 などの判型にならない（この本の環境では 595.5 × 792 pt） |
| 図の id を `<img>` に付けた（VFM の既定） | 図一覧などの `target-text()` でキャプションが取れない |
...
`@vivliostyle/theme-base` の `--vs-page--size` には既定値がありません。何も書かないと `size` は初期値の `auto` になり、判型はブラウザ側で決まります。この本の環境では 595.5 × 792 pt で出ました。設定の `size: 'A5'` で指定するのがいちばん素直です。
...
この本は theme-base を `book.css` の先頭で `@import` しているので、並べ直すのは自分の CSS だけで済みます。
